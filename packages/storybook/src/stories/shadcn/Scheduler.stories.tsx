import type { Meta, StoryObj } from '@storybook/react-vite';
import React, { useState } from 'react';
import { SchedulerProvider, ESchedulerPrefix, type IScheduler } from '@teamco/ischeduler-core';
import { Scheduler } from '@teamco/ischeduler-shadcn';
import { emptySchedulers, mockCrudCallbacks } from '../antd/mocks/scheduler.mocks';
import { ShadcnWrapper } from './shadcn-decorator';

const SchedulerWithState = (args: { schedulerType: ESchedulerPrefix; disabled?: boolean }) => {
  const [value, setValue] = useState<IScheduler | undefined>();

  return (
    <ShadcnWrapper className="max-w-[640px]">
      <div className="rounded-lg border bg-card p-6 shadow-sm">
        <Scheduler
          value={value}
          onChange={setValue}
          schedulerType={args.schedulerType}
          disabled={args.disabled}
        />
      </div>
    </ShadcnWrapper>
  );
};

const meta: Meta = {
  title: 'Shadcn/Scheduler',
  parameters: {
    layout: 'padded',
    docs: {
      description: {
        component:
          'The main scheduling form (shadcn/ui version). Renders duration, start date, repeat behavior (weekly/monthly/yearly), and end reason fields. For DISCOUNT and TRIAL_DISCOUNT types, also shows discount type and value fields. Must be wrapped in a `SchedulerProvider`.',
      },
    },
  },
  decorators: [
    (Story) => (
      <SchedulerProvider schedulers={emptySchedulers} {...mockCrudCallbacks}>
        <Story />
      </SchedulerProvider>
    ),
  ],
};

export default meta;
type Story = StoryObj;

export const SaleScheduler: Story = {
  render: () => <SchedulerWithState schedulerType={ESchedulerPrefix.SALE} />,
  parameters: {
    docs: {
      description: {
        story: 'Sale scheduler form (shadcn/ui) — shows duration, repeat, start date, and end reason fields.',
      },
    },
  },
};

export const DiscountScheduler: Story = {
  render: () => <SchedulerWithState schedulerType={ESchedulerPrefix.DISCOUNT} />,
  parameters: {
    docs: {
      description: {
        story: 'Discount scheduler form (shadcn/ui) — same as sale but with additional discount type and value fields.',
      },
    },
  },
};

export const Disabled: Story = {
  render: () => <SchedulerWithState schedulerType={ESchedulerPrefix.SALE} disabled />,
  parameters: {
    docs: {
      description: {
        story: 'All form fields disabled (shadcn/ui). Use for read-only display.',
      },
    },
  },
};
